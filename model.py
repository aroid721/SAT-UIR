...
class AIMnet(nn.Module):
    def __init__(self, n_feat=32, height=256, width=256, n_RCB=2, chan_factor=2, bias=True):
        ...
        self.head_pseudo = nn.Sequential(
            nn.Linear(self.n_feat*2, self.n_feat*2),
            nn.LeakyReLU(0.1, True),
            nn.Linear(self.n_feat*2, self.n_feat),
            nn.LeakyReLU(0.1, True),
    ...
    def forward(self, x, la):
        ...
        branch = self.gmp(x_top1)
        branch = branch.view(-1, self.n_feat)
        ...
        branch2 = self.gmp(out)
        branch2 = branch2.view(-1, self.n_feat)
        concat_all = torch.cat([branch, branch2], dim=1)

        result = torch.sigmoid(result)
        ssim_pseudo = self.head_pseudo(concat_all)
        return result, res_grad, ssim_pseudo, concat_all
